...
def atrous_identity_block(kernel_size, filters, stage, block, weight_decay=0., atrous_rate=(2, 2), batch_momentum=0.99):
    ...
    b1 = Conv2D(nb_filter1, (1, 1), name=conv_name_base + '2a', kernel_regularizer=l2(weight_decay))
    b2 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a', momentum=batch_momentum)
    b3 = Conv2D(nb_filter2, (kernel_size, kernel_size), dilation_rate=atrous_rate,
                          padding='same', name=conv_name_base + '2b', kernel_regularizer=l2(weight_decay))
    b4 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2b', momentum=batch_momentum)
    b5 = Conv2D(nb_filter3, (1, 1), name=conv_name_base + '2c', kernel_regularizer=l2(weight_decay))
    b6 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2c', momentum=batch_momentum)
    def f(input_tensor):
        x = b1(input_tensor)
        x = b2(x)
        x = Activation('relu')(x)
...
def atrous_conv_block(kernel_size, filters, stage, block, weight_decay=0., strides=(1, 1), atrous_rate=(2, 2), batch_momentum=0.99):
    ...
    nb_filter1, nb_filter2, nb_filter3 = filters
    b1 = Conv2D(nb_filter1, (1, 1), strides=strides,
                name=conv_name_base + '2a', kernel_regularizer=l2(weight_decay))
    b2 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a', momentum=batch_momentum)
    b3 = Conv2D(nb_filter2, (kernel_size, kernel_size), padding='same', dilation_rate=atrous_rate,
                          name=conv_name_base + '2b', kernel_regularizer=l2(weight_decay))
    b4 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2b', momentum=batch_momentum)
    b5 = Conv2D(nb_filter3, (1, 1), name=conv_name_base + '2c', kernel_regularizer=l2(weight_decay))
    b6 = BatchNormalization(axis=bn_axis, name=bn_name_base + '2c', momentum=batch_momentum)
    b7 = Conv2D(nb_filter3, (1, 1), strides=strides,
                                 name=conv_name_base + '1', kernel_regularizer=l2(weight_decay))
    b8 = BatchNormalization(axis=bn_axis, name=bn_name_base + '1', momentum=batch_momentum)
    def f(input_tensor):
        x = b1(input_tensor)
        x = b2(x)
        x = Activation('relu')(x)
        ...
        shortcut = b7(input_tensor)
        shortcut = b8(shortcut)
